...
class SourceCodeCleanerAndFormatter:
    ...
    def contains_curly_brackets_with_content(self,line):
        # Define a pattern to match curly brackets with content
        pattern = r'\{[^\}]*\}'
    ...
    def format_solidity_code(self, input_code):
        lines = input_code.split(";")
        lines = ";\n".join(lines)
        lines = lines.split("\n")
        for i in range(len(lines)):
            # Add newline after opening curly brace
            if "{" in lines[i] :
                arr = lines[i].split("{")
                for j in range(len(arr)):
                    if "}" not in arr[j]:
                        arr[j] = '\n' + arr[j]
                lines[i] = '{'.join(arr)
            if "}" in lines[i] and self.contains_curly_brackets_with_content(lines[i]) == False:
                lines[i] = '}\n'.join(lines[i].split('}'))
        lines = '\n'.join(lines)
